...
class Plot:
    ...
    def __str__(self):
        ...
        status = (
            "已成熟"
            if self.is_mature()
            else f"生長中({self.progress}/{self.crop['steps']})"
        )
...
class FarmingSystem:
    ...
    def hunt_monsters(self):
        ...
        if random.random() < drop_rate:
            ...
            print(
                f"✨ 額外驚喜！你從怪物身上搜到了 {extra_fertilizer} 個額外肥料！(目前機率: {drop_rate*100:.2f}%)"
            )
    ...
    def manage_farm(self):
        while True:
            ...
            # 在介面上顯示當前等級與經驗值
            print(
                f"🌾 我的農場 | 等級: {self.level} (EXP: {self.exp}/3) | 金幣: {self.money} | 肥料: {self.fertilizer_count}"
            )
            for i, plot in enumerate(self.plots):
                print(f"{i+1}. {plot}")
            print("=" * 30)
            print(
                "指令: 1-3(選擇土地) | s(商店) | h(打怪) | m(賣出) | n(下一天/生長) | q(離開)"
            )
            ...
            if cmd in ["1", "2", "3"]:
                ...
                if plot.is_mature():
                    ...
                    print("選擇種子:")
                    seeds = [
                        k for k, v in self.inventory.items() if "種子" in k and v > 0
                    ]
                    if not seeds:
                        print("你沒有任何種子。")
                    else:
                        ...
                        if s_choice.isdigit() and 1 <= int(s_choice) <= len(seeds):
                            ...
                            c_type = None
                            if "小麥" in s_name:
                                c_type = CropType.WHEAT
                            elif "玉米" in s_name:
                                c_type = CropType.CORN
                            elif "草莓" in s_name:
                                c_type = CropType.STRAWBERRY

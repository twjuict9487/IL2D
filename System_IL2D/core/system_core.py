...
from core.functions.ui.tutorial_flow import build_tutorial_lines as _ui_build_tutorial_lines,handle_tutorial_key as _ui_handle_tutorial_key,handle_tutorial_mouse as _ui_handle_tutorial_mouse
from core.functions.ui.lore_flow import load_lore_archive as _ui_load_lore_archive,build_lore_index as _ui_build_lore_index,get_lore_entry_by_id as _ui_get_lore_entry_by_id,get_lore_entry_pages as _ui_get_lore_entry_pages,get_lore_entry_title as _ui_get_lore_entry_title
from core.functions.ui.continue_flow import get_save_slots as _ui_get_save_slots,handle_continue_menu_key as _ui_handle_continue_menu_key,handle_mouse_continue_menu as _ui_handle_mouse_continue_menu
from core.functions.ui.main_menu_flow import handle_main_menu_key as _ui_handle_main_menu_key,handle_mouse_main_menu as _ui_handle_mouse_main_menu
from core.functions.ui.settings_flow import handle_settings_key as _ui_handle_settings_key,handle_mouse_settings as _ui_handle_mouse_settings
from core.functions.ui.name_input_flow import open_new_game_name_input as _ui_open_new_game_name_input,handle_name_input_key as _ui_handle_name_input_key,handle_text_input as _ui_handle_text_input
from core.functions.ui.dev_menu_flow import handle_dev_menu_key as _ui_handle_dev_menu_key
from core.functions.input.held_movement import update_held_keys as _input_update_held_keys,press_move as _input_press_move,handle_held_movement as _input_handle_held_movement
...
def _set_always_on_top():
	try:
		...
		SWP_NOMOVE=2
		SWP_NOACTIVATE=16
		ctypes.windll.user32.SetWindowPos(int(hwnd),HWND_TOPMOST,0,0,0,0,SWP_NOSIZE|SWP_NOMOVE|SWP_NOACTIVATE)
	except Exception:
		pass
...
def init_context():
	...
	ctx={'screen':screen,'clock':clock,'game':Game(),'running':True,'fullscreen':False,'state':'main_menu','new_game_name':'','cutscene_lines':[],'cutscene_idx':0,'lore_archive':_ui_load_lore_archive(),'lore_mode':None,'lore_entry_id':None,'lore_pages':[],'lore_page_idx':0,'lore_reveal_chars':0.0,'lore_reveal_speed':120.0,'lore_reveal_done':False,'lore_return_state':'game','lore_entry_title':'','lore_index_entries':[],'lore_index_selected':0,'tutorial_lines':[],'tutorial_idx':0,'tutorial_return_state':'game','tutorial_mode':'start','menu_selected':0,'settings_selected':0,'settings_sub':None,'lang_selected':0,'esc_selected':0,'continue_selected':0,'continue_slots':[],'type_buffer':'','dev_menu_selected':0,'dev_menu_target':None,'dev_menu_input':'','held_keys':{'w':False,'a':False,'s':False,'d':False},'press_time':{'w':0.0,'a':0.0,'s':0.0,'d':0.0},'last_move_time':0.0,'move_interval':0.133,'hold_repeat_delay':0.08,'world_tick_interval':0.5,'world_tick_accum':0.0,'loaded_mods':[],'mod_errors':[]}
	try:
...
def _start_lore_index(ctx,game,return_state='game'):
	...
	ctx['lore_index_selected']=min(max(0,int(ctx.get('lore_index_selected',0))),max(0,len(ctx.get('lore_index_entries',[]))-1))
	ctx['state']='lore_reader'
...
def _update(ctx,dt):
	...
	if ctx.get('state')=='game'and getattr(game,'startup_closure_pending',False)and getattr(game,'ui_mode',None)is None:
...
def _render(ctx):
	if ctx['state']=='main_menu':
		...
		draw_settings_menu(ctx['screen'],ctx['settings_selected'],ctx['settings_sub'],ctx['lang_selected'],ctx['game'].lang)

...
from core.functions.rendering.draw import (
    draw,
    draw_main_menu,
    draw_esc_menu,
    draw_player_ui,
...
def _has_seen_start_tutorial():
    try:
        ...
        return (
            bool(data.get("start_tutorial_seen", False))
...
def init_context():
    ...
    try:
        ...
        ctx["hold_repeat_delay"] = cfg.get(
            "hold_repeat_delay", ctx["hold_repeat_delay"]
        )
    except Exception:
        pass
...
def _handle_mouse_scroll(ctx, button):
    ...
    if game.ui_mode == "dialog":
        ...
        if hasattr(game, "mission_detail_scroll"):
            game.mission_detail_scroll = max(
                0, int(getattr(game, "mission_detail_scroll", 0) or 0) + step
            )
    elif game.ui_mode == "interact_pick":
        step = -1 if button == 4 else 1
        game.interact_scroll = max(
            0, int(getattr(game, "interact_scroll", 0) or 0) + step
        )
...
def _start_lore_entry(ctx, game, entry_id="opening_short", return_state="game"):
    ...
    if entry:
        ctx["lore_entry_title"] = _ui_get_lore_entry_title(
            entry, game.lang if game else "zh"
        )
    else:
    ...
    if mode == "index":
        ...
        if entry_id:
            _start_lore_entry(
                ...
                game,
                entry_id=entry_id,
                return_state=ctx.get("lore_return_state", "game"),
            )
        return
...
def _update_lore_reader(ctx, dt):
    ...
    ctx["lore_reveal_chars"] = min(
        len(page_text),
        float(ctx.get("lore_reveal_chars", 0.0))
        + max(1.0, speed) * max(0.0, float(dt)),
    )
...
def _handle_lore_key(ctx, event):
    ...
    if ctx.get("lore_mode") == "index":
        ...
        if event.key in (pygame.K_UP, pygame.K_w):
            ctx["lore_index_selected"] = max(
                0, int(ctx.get("lore_index_selected", 0)) - 1
            )
            return
        if event.key in (pygame.K_DOWN, pygame.K_s):
            ctx["lore_index_selected"] = min(
                len(entries) - 1, int(ctx.get("lore_index_selected", 0)) + 1
            )
            return
...
def _draw_lore_reader(ctx):
    ...
    if mode == "index":
        ...
        hint = hint_font.render(
            tr(game.lang, "lore.archive.hint"), True, (170, 205, 230)
        )
        screen.blit(hint, (panel.right - hint.get_width() - 20, panel.y + 20))
        entries = ctx.get("lore_index_entries", [])
        if not entries:
            empty = body_font.render(
                tr(game.lang, "lore.archive.empty"), True, (230, 238, 248)
            )
        ...
        for i, entry in enumerate(entries):
            rect = pygame.Rect(
                panel.x + 16, y - 2, panel.width - 32, body_font.get_height() + 10
            )
        ...
        entry = _ui_get_lore_entry_by_id(
            ctx.get("lore_archive", {}), ctx.get("lore_entry_id")
        )
        pages = ctx.get("lore_pages", []) or [""]
        page_idx = max(0, min(len(pages) - 1, int(ctx.get("lore_page_idx", 0))))
        ...
        title_text = ctx.get("lore_entry_title") or (
            _ui_get_lore_entry_title(entry, game.lang)
            if entry
            else tr(game.lang, "lore.entry.title")
        )
        title_surf = title_font.render(title_text, True, (230, 245, 255))
        screen.blit(title_surf, (panel.x + 20, panel.y + 16))
        page_text_surf = hint_font.render(
            f"{page_idx+1}/{len(pages)}", True, (170, 205, 230)
        )
        screen.blit(
            page_text_surf,
            (panel.right - page_text_surf.get_width() - 20, panel.y + 20),
        )
        y = panel.y + 76
        ...
        if int(ctx.get("lore_page_idx", 0)) + 1 >= len(pages) and ctx.get(
            "lore_reveal_done", False
        ):
        ...
        screen.blit(
            hint,
            (
                panel.right - hint.get_width() - 20,
                panel.bottom - hint.get_height() - 16,
            ),
        )
...
def _handle_hotbar_menu_key(game, event):
    ...
    def _sync_hotbar_selected_to_mode():
        if game.hotbar_mode == "item":
            game.hotbar_item_list_selected = max(
                0, int(getattr(game, "hotbar_list_selected", 0))
            )
        else:
            game.hotbar_magic_list_selected = max(
                0, int(getattr(game, "hotbar_list_selected", 0))
            )
...
def _handle_esc_menu_key(ctx, event):
    ...
    if game.ui_mode:
        ...
        if game.ui_mode == "team":
            members = (
                game.get_team_member_ids()
                if hasattr(game, "get_team_member_ids")
                else []
            )
        ...
        if event.key in (pygame.K_UP, pygame.K_LEFT, pygame.K_w, pygame.K_a):
            if game.ui_mode == "save":
                ...
                game.team_equip_root_selected = max(
                    0, game.team_equip_root_selected - 1
                )
                ...
                if focus == "tabs":
                    ...
                    slot_key = (
                        "ring"
                        if game.equip_category.startswith("ring")
                        else game.equip_category
                    )
                    filtered = [
                        n
                        for n in equipables
                        if game.item_defs.get(n, {}).get("slot") == slot_key
                    ]
                    if not filtered:
                        return
                ...
                game.team_equip_slot_selected = max(
                    0, game.team_equip_slot_selected - 1
                )
                ...
                if game.ui_mode == "objective":
                    ...
                    missions = (
                        game.get_mission_board_entries(
                            getattr(game, "mission_board_giver", None)
                        )
                        if hasattr(game, "get_mission_board_entries")
                        else []
                    )
                    if missions:
                        game.mission_board_selected = max(
                            0, game.mission_board_selected - 1
                        )
            ...
            if game.ui_mode == "save":
                ...
                game.team_equip_root_selected = min(
                    2, game.team_equip_root_selected + 1
                )
            elif game.ui_mode == "equip":
                focus = getattr(game, "equip_focus", "tabs")
                if focus == "tabs":
                ...
                game.equip_category_selected = min(
                    max_idx, game.equip_category_selected + 1
                )
            elif game.ui_mode == "team_equip_category":
                max_idx = len(game.get_team_equip_categories()) - 1
                game.team_equip_slot_selected = min(
                    max_idx, game.team_equip_slot_selected + 1
                )
            elif game.ui_mode == "item":
                focus = getattr(game, "item_focus", "tabs")
                if focus == "tabs":
                ...
                if game.ui_mode == "objective":
                    ...
                    if missions:
                        game.objective_selected = min(
                            len(missions) - 1, game.objective_selected + 1
                        )
                else:
                    missions = (
                        game.get_mission_board_entries(
                    ...
                    if missions:
                        game.mission_board_selected = min(
                            len(missions) - 1, game.mission_board_selected + 1
                        )
            elif game.ui_mode == "hotbar":
                if event.key in (pygame.K_RIGHT, pygame.K_d):
                    game.hotbar_slot_selected = min(9, game.hotbar_slot_selected + 1)
            ...
            if game.ui_mode == "save":
                ...
                cats = game.get_team_equip_categories()
                game.team_equip_category = cats[
                    game.team_equip_slot_selected % len(cats)
                ]
                ...
                if missions:
                    idx = max(
                        ...
                        min(
                            len(missions) - 1,
                            int(getattr(game, "mission_board_selected", 0)),
                        ),
                    )
                    row = missions[idx]
                ...
                if game.hotbar_mode == "item":
                    ...
                    if spells:
                        ...
                        game.magic_hotbar_slots[game.hotbar_slot_selected] = sp.get(
                            "name"
                        )
                        game.tutorial_notify(
                            "hotbar_magic_assigned", spell_name=sp.get("name")
                        )
            elif game.ui_mode == "skill_tree":
...
def _handle_mouse_main_menu(ctx, pos):
    _ui_handle_mouse_main_menu(
        ctx, pos, _get_font, _open_new_game_name_input, _get_save_slots
    )
...
def _handle_mouse_esc_menu(ctx, pos):
    ...
    if mx < menu_w:
        ...
        if game.ui_mode == "story" and getattr(game, "story_graph_view", "graph") == "graph":
            ...
            rings = sorted(
                [c for c in cats if c.startswith("ring")],
            ...
            for r in range(cat_rows):
                for c in range(2):
                    ...
                    slot_rect = pygame.Rect(
                        base_x, ry - 2, slot_w, font.get_height() + 6
                    )
            ...
            tab_gap = 6
            tab_w = max(
                80, (panel.width - 32 - (len(cats) - 1) * tab_gap) // max(1, len(cats))
            )
            tab_y = panel.y + 48
            ...
            item_tab = pygame.Rect(
                panel.x + 16, tab_y - 2, tab_w, font.get_height() + 8
            )
            magic_tab = pygame.Rect(
                panel.x + 20 + tab_w, tab_y - 2, tab_w, font.get_height() + 8
            )
            ...
            if game.ui_mode == "objective":
                ...
                left = pygame.Rect(
                    panel.x + 14, panel.y + 52, left_w, panel.height - 66
                )
...
def _handle_mouse_game(ctx, pos):
    ...
    if game.ui_mode == "death_menu":
        ...
        for i in range(2):
            rect = pygame.Rect(
                panel.x + 24, start_y + i * (row_h + 10), panel.width - 48, row_h
            )
            if rect.collidepoint(mx, my):
    ...
    if game.ui_mode == "dialog":
        ...
        if text_source is None:
            node = _resolve_dialog_node(
                getattr(game, "dialog_data", None), getattr(game, "dialog_node", None)
            )
            if isinstance(node, dict):
                text_source = (
                    node.get("text_zh", node.get("text", ""))
                    if game.lang == "zh"
                    else node.get("text", node.get("text_zh", ""))
                )
            else:
                text_source = ""
        ...
        text_bottom = right.bottom - (
            opt_row_h * reserve_rows + 18 if reserve_rows else 18
        )
        max_text_lines = max(
            3, (text_bottom - right.y - 4) // (body_font.get_height() + 4)
        )
        ...
        yy = (
            right.y
            + 4
            + len(dialog_lines[start : start + max_text_lines])
            * (body_font.get_height() + 4)
        )
        visible_rows = (
            max(2, (right.bottom - max(yy + 10, right.y + 88) - 18) // opt_row_h)
            if responses
            else 0
        )
        selected = int(getattr(game, "dialog_selected", 0) or 0)
        start_opt, end_opt = (
            _ui_visible_range(len(responses), selected, visible_rows)
            if responses
            else (0, 0)
        )
        ...
        if responses:
            start_opt = max(
                0, min(start_opt + opt_scroll, max(0, len(responses) - visible_rows))
            )
...
def _update(ctx, dt):
    ...
    if (
        ctx.get("state") == "game"
        and getattr(game, "startup_closure_pending", False)
        and getattr(game, "ui_mode", None) is None
    ):
        game.maybe_startup_closure_greet()
        game.startup_closure_pending = False
...
def _draw_name_input(ctx):
    ...
    pygame.draw.rect(
        screen, (16, 30, 46), (box_x, box_y, box_w, box_h), border_radius=8
    )
    pygame.draw.rect(
        screen, (190, 230, 255), (box_x, box_y, box_w, box_h), 2, border_radius=8
    )
    ...
    screen.blit(
        hint, ((screen.get_width() - hint.get_width()) // 2, box_y + box_h + 18)
    )
...
def _draw_cutscene(ctx):
    ...
    screen.blit(
        hint,
        (panel.right - hint.get_width() - 16, panel.bottom - hint.get_height() - 10),
    )
...
def _draw_tutorial(ctx):
    ...
    screen.blit(
        hint,
        (panel.right - hint.get_width() - 20, panel.bottom - hint.get_height() - 16),
    )

...
from .map import GameMap, blocktypes, mobs_data, player_data
...
class Game:
    ...
    def cast_spell(self):
        ...
        if name == "heal":
            ...
            if healed <= 0:
                self.push_message("heal: HP full")
            else:
                self.push_message(f"heal +{healed}")
        else:

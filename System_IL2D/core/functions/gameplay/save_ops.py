...
def load_save(game, slot):
    ...
    if isinstance(saved_inv, dict):
        ...
        if (
            "rouge level skipper" in saved_inv
            and "rogue level skipper" not in saved_inv
        ):
    ...
    game.tracked_mission = data.get(
        "tracked_mission", getattr(game, "tracked_mission", None)
    )
    game.objective_selected = int(
        data.get("objective_selected", getattr(game, "objective_selected", 0))
    )
    loaded_mission_state = data.get("mission_state", None)
    if isinstance(loaded_mission_state, dict):
        game.mission_state = game_missions.normalize_state(
            loaded_mission_state, getattr(game, "mission_book", None)
        )
    else:
        legacy_state = {
            "active": {
                str(m.get("id", f"legacy_{i}")): m
                for i, m in enumerate(getattr(game, "active_missions", []))
                if isinstance(m, dict)
            },
            "accepted": [
                str(m.get("id", f"legacy_{i}"))
                for i, m in enumerate(getattr(game, "active_missions", []))
                if isinstance(m, dict)
            ],
            "completed": [],
            ...
            "completed_count": int(
                data.get("mission_complete_count", data.get("kaltsit_completed", 0))
                or 0
            ),
        }
        game.mission_state = game_missions.normalize_state(
            legacy_state, getattr(game, "mission_book", None)
        )
    state_ref = (
        getattr(game, "mission_state", {})
        if isinstance(getattr(game, "mission_state", {}), dict)
        else {}
    )
    game.mission_key_items = data.get(
        "mission_key_items",
        state_ref.get("key_items", getattr(game, "mission_key_items", {})),
    )
    game.mission_flags = data.get(
        "mission_flags", state_ref.get("flags", getattr(game, "mission_flags", {}))
    )
    game.mission_board_giver = data.get(
        "mission_board_giver",
        state_ref.get("board_giver", getattr(game, "mission_board_giver", None)),
    )
    game.mission_complete_count = int(
        data.get(
            ...
            state_ref.get(
                ...
                data.get(
                    "kaltsit_completed",
                    (
                        game.mission_complete_count
                        if hasattr(game, "mission_complete_count")
                        else 0
                    ),
                ),
            ),
    ...
    game.kaltsit_reward_ready = data.get(
        "kaltsit_reward_ready", game.kaltsit_reward_ready
    )
    ...
    game._normalize_team_equipment(
        data.get("team_equipment", getattr(game, "team_equipment", {}))
    )
    ...
    if isinstance(item_slots, list):
        game.item_hotbar_slots = [
            (game.canonical_item_name(v) if v else None)
            for v in (item_slots + [None] * 10)[:10]
        ]
    magic_slots = data.get("magic_hotbar_slots")
    if isinstance(magic_slots, list):
        game.magic_hotbar_slots = [
            (game.canonical_spell_name(v) if v else None)
            for v in (magic_slots + [None] * 10)[:10]
        ]
    ...
    if isinstance(loaded_unlock, list):
        game.unlocked_magics = [
            game.canonical_spell_name(v) for v in loaded_unlock if isinstance(v, str)
        ]
    game.level_stat_pending = int(
        data.get("level_stat_pending", getattr(game, "level_stat_pending", 0))
    )

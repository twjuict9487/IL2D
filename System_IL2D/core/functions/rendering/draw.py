...
_SPELL_ATLAS_CACHE={}
_TILESET_CACHE={}
_DIALOG_NPC_FALLBACK_IMAGE={'dev':'noFilter_nobg.png','priestess':'priestess_nobg.png','carmen':'carmen_nobg.png','closure':'Closure_nobg.png','kaltsit':'憭游?_?臬?撣?png','ines':'憭游?_隡\uf4c0?銝?png','monst3r':'憭游?_Mon3tr.png','wisadel':'憭游?_蝏港??游?.png','shu':'憭游?_暺?png'}
def _a(bt_meta,default=0.9):
	try:
		...
		if isinstance(exact_tiles,list):
			for item in exact_tiles:
				...
				try:
					...
					rect=(int(item.get('x',0)or 0),int(item.get('y',0)or 0),int(item.get('w',0)or 0),int(item.get('h',0)or 0))
		...
		for item in selected:
			if item.get('name')==ref_name:
				try:
					...
					if target is None:
						target=(int(item.get('x',0)),int(item.get('y',0)),int(item.get('w',0)),int(item.get('h',0)))
				except Exception:
					target=None
				break
		if not target:
...
def _d(folder_name,size=None):
	...
	return frames
def _e(folder_name,size=None,fps=12):
	frames=_d(folder_name,size)
	if not frames:
		return None
...
def _g(atlas_cfg):
	...
	return out
def _h(game,screen,cam_px,cam_py,tile_w,tile_h):
...
def _j(game,ent,ent_def,size):
	...
	if render_mode=='animated':
		...
		if state not in('move','idle')and ent_def.get(state_key):
			img=_e(ent_def.get(state_key),size=size,fps=fps)
			if img is not None:
				return img
		if state=='skill3'and ent_def.get('anim_skill3'):
			img=_e(ent_def.get('anim_skill3'),size=size,fps=fps)
			if img is not None:
				return img
		if ent_def.get('anim_move'):
			img=_e(ent_def.get('anim_move'),size=size,fps=fps)
			if img is not None:
				return img
	return _b(ent_def.get('image'),size)
def _k(game,npc_id,ent_def,size):
...
def _n(game):
	...
	base=_m(top_block)
	return(min(255,int(base[0]*0.9+12)),min(255,int(base[1]*0.9+12)),min(255,int(base[2]*0.9+12)))
def _o(screen,center_x,center_y,outer_r,fill_color,edge_color):
...
def _p(game,screen,cam_px,cam_py,view_w_px,view_h_px):
	...
	return mini_rect
def _q(size,bold=False):
	for font_path in('C:\\Windows\\Fonts\\msjh.ttc','C:\\Windows\\Fonts\\msyh.ttc','C:\\Windows\\Fonts\\mingliu.ttc'):
...
def _r(screen,font,text,color,outline_color,pos,thickness=1):
	x,y=pos
...
def _s(screen,rect,selected=False):
...
def draw_main_menu(screen,selected,lang='zh'):
	...
	for i,opt in enumerate(opts):
		...
		if i==selected:
			...
			pygame.draw.rect(screen,_l(),rect,2,border_radius=6)
...
def draw_dev_menu(screen,ctx):
	...
	labels={'pre_dev_set':tr(lang,'dev.pre_dev_set'),'max_hp':tr(lang,'dev.max_hp'),'max_mp':tr(lang,'dev.max_mp'),'add_money':tr(lang,'dev.add_money'),'add_skipper':tr(lang,'dev.add_skipper'),'get_dev_set':tr(lang,'dev.get_dev_set'),'exit':tr(lang,'dev.exit')}
	y=panel.y+60
...
def draw_esc_menu(screen,selected,game=None):
	...
	for i,opt in enumerate(opts):
		...
		is_entered=bool(game is not None and is_selected and _y(game,opt))
		...
		txt_y=item_rect.y+2
		_r(screen,font,label,color,(0,0,0),(txt_x,txt_y),thickness=2)
	right_x=menu_w
	...
	if game is not None:
		...
		_r(screen,font,name,(255,255,255),(0,0,0),(right_x-name_surf.get_width(),header_rect.y+10))
		_r(screen,font2,line1,(255,235,210),(255,255,255),(right_x-line1_surf.get_width(),header_rect.y+32))
		_r(screen,font2,line2,(210,235,255),(255,255,255),(right_x-line2_surf.get_width(),header_rect.y+48))
...
def _z(screen,panel,game,label):
...
def draw_menu_preview(screen,panel,game,selected):
	...
	if label=='item':
		for name,count in list(game.inventory.items()):
			if count<=0:
				continue
			lines.append(f'{_u(game,name)} x{count}')
	elif label=='hotbar':
		...
		for i,v in enumerate(slots):
			...
			name=_u(game,v)if game.active_hotbar=='item'else _v(game,v)
		...
		for slot in game.get_equip_categories():
			...
			equip_label=_u(game,equipped)if equipped else tr(game.lang,'label.none')
			lines.append(f'{_w(game,slot)}: {equip_label}')
			shown+=1
	elif label=='team':
	...
	if label=='item':
		...
		for i,line in enumerate(lines):
			...
			yy=y+row*row_h
			line=_t(font,line,col_w-4)
			_r(screen,font,line,(235,235,235),(0,0,0),(x,yy))
			shown+=1
		return
	for line in lines:
		line=_t(font,line,panel.width-36)
		_r(screen,font,line,(235,235,235),(0,0,0),(panel.x+20,y))
		y+=font.get_height()+6
def draw_menu_detail(screen,panel,game):
	...
	if game.ui_mode=='item':
		...
		for i,cat in enumerate(categories):
			...
			_s(screen,rect,selected=tab_active)
			...
			label=_t(font,_x(game,cat),tab_w-10)
			color=(255,247,170)if tab_active else(230,230,230)
			_r(screen,font,label,color,(0,0,0),(rx+6,tab_y),thickness=2)
		y+=tab_h+10
		items=game.get_item_list()
		if not items:
			_r(screen,font,tr(game.lang,'msg.no_items'),(230,230,230),(255,255,255),(panel.x+20,y))
			return
		...
		for i,name in enumerate(show):
			...
			_s(screen,rect,selected=item_active)
			color=(255,247,170)if item_active else(230,230,230)
			_r(screen,font,_t(font,line,col_w-10),color,(0,0,0),(rx+6,ry),thickness=2)
		...
		head_y=y
		_r(screen,font,tr(game.lang,'hotbar.item'),(255,247,170)if mode=='item'else(230,230,230),(0,0,0),(panel.x+20,head_y),thickness=2)
		_r(screen,font,tr(game.lang,'hotbar.magic'),(255,247,170)if mode=='magic'else(230,230,230),(0,0,0),(right_x+4,head_y),thickness=2)
		y+=font.get_height()+8
		rows=list(range(10))
		row_h=font.get_height()+8
		for i in rows:
			...
			_s(screen,left_rect,selected=left_active)
			_s(screen,right_rect,selected=right_active)
			...
			left_txt=f"{key}: {(_u(game,item_name)if item_name else'-')}"
			right_txt=f"{key}: {(_v(game,magic_name)if magic_name else'-')}"
			...
			_r(screen,font,_t(font,left_txt,left_w-10),left_color,(0,0,0),(left_rect.x+6,ly),thickness=2)
			_r(screen,font,_t(font,right_txt,left_w-10),right_color,(0,0,0),(right_rect.x+6,ly),thickness=2)
			y+=row_h
		if stage=='grid':
			hint='W/S: row  A/D: item/magic  Enter: choose  Del: clear'
			_r(screen,font,hint,(200,200,200),(255,255,255),(panel.x+20,panel.bottom-28))
			return
		...
		_r(screen,font,title,(230,230,230),(255,255,255),(panel.x+20,picker_top))
		picker_y=picker_top+font.get_height()+8
		if not src:
			_r(screen,font,tr(game.lang,'msg.no_items'if mode=='item'else'msg.no_spells'),(230,230,230),(255,255,255),(panel.x+20,picker_y))
			return
		...
		for i in range(start,end):
			...
			_s(screen,rect,selected=i==sel)
			label=_u(game,src[i])if mode=='item'else _v(game,src[i])
			_r(screen,font,_t(font,label,panel.width-44),(255,247,170)if i==sel else(230,230,230),(0,0,0),(panel.x+24,picker_y),thickness=2)
			picker_y+=list_row_h
	elif game.ui_mode=='equip_root':
		options=[tr(game.lang,'equip.change'),tr(game.lang,'equip.best'),tr(game.lang,'equip.put_down_all')]
		...
		for i,line in enumerate(options):
			...
			if i==selected:
				pygame.draw.rect(screen,_l(),rect,2,border_radius=4)
			_r(screen,font,line,(230,230,230),(255,255,255),(panel.x+24,y))
			y+=font.get_height()+6
	elif game.ui_mode=='equip':
		tabs=[tr(game.lang,'equip.change'),tr(game.lang,'equip.best'),tr(game.lang,'equip.put_down_all')]
		...
		for i,t in enumerate(tabs):
			...
			_r(screen,font,_t(font,t,tab_w-10),tab_color,(0,0,0),(rx+6,tab_y),thickness=2)
		y+=font.get_height()+14
		categories=game.get_equip_categories()
		non_rings=[c for c in categories if not c.startswith('ring')]
		rings=sorted([c for c in categories if c.startswith('ring')],key=lambda s:int(s[4:])if s[4:].isdigit()else 99)
		...
		for r in range(cat_rows):
			for c in range(2):
				...
				_s(screen,slot_rect,selected=is_active)
				_s(screen,val_rect,selected=is_active)
				...
				_r(screen,font,_t(font,label,slot_w-10),slot_color,(0,0,0),(slot_rect.x+6,ry),thickness=2)
				_r(screen,font,_t(font,equip_label,val_w-10),(230,230,230),(0,0,0),(val_rect.x+6,ry),thickness=2)
		y+=cat_rows*row_h+10
		if categories:
		...
		for i in range(start,end):
			...
			_s(screen,rect,selected=item_active)
			item_color=color
			_r(screen,font,_t(font,label,col_w2-10),item_color,(0,0,0),(rx+8,ry))
		...
		for i,name in enumerate(categories):
			...
			if is_sel:
				pygame.draw.rect(screen,_l(),left_rect,2,border_radius=4)
			equipped=game.equipment.get(name)if name in game.equipment else None
			equip_label=_u(game,equipped)if equipped else tr(game.lang,'label.none')
			_r(screen,font,label,color,(255,255,255),(left_rect.x+8,y))
			_r(screen,font,equip_label,(230,230,230),(255,255,255),(right_rect.x+8,y))
			y+=font.get_height()+6
		...
		for i in range(3):
			...
			count=game.inventory.get(name,0)
			line=f'{_u(game,name)} x{count}'
			_r(screen,font,line,(230,230,230),(255,255,255),(panel.x+16,bottom_y+i*(font.get_height()+6)))
	elif game.ui_mode=='team':
		members=getattr(game,'team_members',[])
		_r(screen,font,tr(game.lang,'preview.team'),(230,230,230),(255,255,255),(panel.x+20,y))
		y+=font.get_height()+10
		if not members:
			_r(screen,font,tr(game.lang,'team.none'),(230,230,230),(255,255,255),(panel.x+20,y))
		else:
			...
			for i,m in enumerate(members):
				...
				_r(screen,font,_t(font,line,panel.width-52),color,(0,0,0),(panel.x+24,y),thickness=2)
				y+=font.get_height()+8
			y+=8
			_r(screen,font,tr(game.lang,'team.hint_open_equip'),(200,220,240),(255,255,255),(panel.x+20,y))
		...
		_r(screen,font,tr(game.lang,'team.equip_title',member=member),(230,230,230),(255,255,255),(panel.x+20,y))
		y+=font.get_height()+10
		tab_w=(panel.width-40)//3
		tab_y=y
		for i,t in enumerate(tabs):
		...
		for i in range(start,end):
			...
			_s(screen,rect,selected=item_active)
			_r(screen,font,_t(font,label,col_w2-10),color,(0,0,0),(rx+8,ry))
		...
		_r(screen,font,title,(210,230,255),(255,255,255),(panel.x+20,y))
		y+=font.get_height()+8
		if not missions:
			_r(screen,font,tr(game.lang,'mission.board.no_missions'),(220,220,220),(255,255,255),(panel.x+20,y))
		else:
			...
			_r(screen,font,tr(game.lang,'mission.board.list'),(210,230,255),(255,255,255),(left_x,y))
			_r(screen,font,tr(game.lang,'mission.board.detail'),(210,230,255),(255,255,255),(right_x,y))
			y+=font.get_height()+8
			list_y=y
			...
			for i,row in enumerate(missions):
				...
				color=(255,247,170)if i==selected else(230,230,230)
				_r(screen,font,_t(font,line,left_w-20),color,(0,0,0),(left_x+8,list_y+i*list_font_gap),thickness=2)
			detail=missions[selected]
			detail_y=list_y
			detail_bottom=panel.bottom-font.get_height()-26
			detail_truncated=False
			sections=[(tr(game.lang,'mission.board.briefing'),detail.get('briefing',[])),(tr(game.lang,'preview.objectives'),detail.get('objectives',[])),(tr(game.lang,'mission.board.rewards'),detail.get('rewards',[])),(tr(game.lang,'mission.board.completion'),detail.get('return_lines',[]))]
			...
			_r(screen,font,_t(font,f"{detail.get('title','')} [{status_line}]",right_w),(255,255,160),(0,0,0),(right_x,detail_y),thickness=2)
			detail_y+=font.get_height()+8
			for heading,lines in sections:
				_r(screen,font,heading,(210,230,255),(255,255,255),(right_x,detail_y))
				detail_y+=font.get_height()+5
				if not lines:
					_r(screen,font,'-',(200,200,200),(255,255,255),(right_x+10,detail_y))
					detail_y+=font.get_height()+4
					continue
				for raw in lines[:6]:
					text=str(raw)
					wrapped=_ab(font,text,right_w-16)
					for wrap_line in wrapped[:3]:
						if detail_y+font.get_height()+6>detail_bottom:
							detail_truncated=True
							break
						_r(screen,font,wrap_line,(230,230,230),(0,0,0),(right_x+10,detail_y),thickness=2)
				...
				detail_y+=4
			if detail_truncated:
				_r(screen,font,'↓ More',(180,205,225),(255,255,255),(right_x,panel.bottom-font.get_height()-18),thickness=2)
			_r(screen,font,tr(game.lang,'mission.board.hint'),(180,200,220),(255,255,255),(right_x,min(panel.bottom-font.get_height()-24,detail_y+8)))
	elif game.ui_mode=='objective':
		for line in game.get_objective_lines():
			_r(screen,font,line,(230,230,230),(255,255,255),(panel.x+20,y))
			y+=font.get_height()+6
		y+=8
		...
		_r(screen,font,missions_title,(210,230,255),(255,255,255),(panel.x+20,y))
		y+=font.get_height()+6
		if not missions:
			empty_text=tr(game.lang,'mission.board.no_missions')or'no missions'
			_r(screen,font,empty_text,(220,220,220),(255,255,255),(panel.x+20,y))
		else:
			...
			for i,row in enumerate(missions):
				...
				rect=pygame.Rect(panel.x+16,y-2,panel.width-32,font.get_height()+6)
				_s(screen,rect,selected=i==selected)
				_r(screen,font,_t(font,line,panel.width-44),(230,230,230),(0,0,0),(panel.x+24,y),thickness=2)
				y+=font.get_height()+8
			if truncated:
				_r(screen,font,'↓ More',(180,205,225),(255,255,255),(panel.x+24,panel.bottom-font.get_height()-18),thickness=2)
			...
			if y+font.get_height()*2+18<panel.bottom-12:
				y+=8
				_r(screen,font,detail_name,(255,247,170),(255,255,255),(panel.x+20,y),thickness=2)
				y+=font.get_height()+4
				if status_line:
					_r(screen,font,status_line,(200,220,240),(255,255,255),(panel.x+20,y),thickness=2)
					y+=font.get_height()+4
				if detail_text:
					for chunk in _ab(font,detail_text,panel.width-40):
						if y+font.get_height()+8>panel.bottom-18:
							_r(screen,font,'↓ More',(180,205,225),(255,255,255),(panel.x+20,panel.bottom-font.get_height()-18),thickness=2)
							break
						_r(screen,font,chunk,(230,230,230),(255,255,255),(panel.x+20,y),thickness=2)
						y+=font.get_height()+4
	elif game.ui_mode=='skill_tree':
		nodes=game.get_skill_tree_nodes()
		if not nodes:
			_r(screen,font,tr(game.lang,'msg.no_skills'),(230,230,230),(255,255,255),(panel.x+20,y))
			return
		_r(screen,font,f"{tr(game.lang,'label.sp')}: {game.player_skill_points}",(230,230,230),(255,255,255),(panel.x+20,y))
		y+=font.get_height()+10
		...
		for i,n in enumerate(nodes):
			...
			rect=pygame.Rect(panel.x+16,y-2,panel.width-32,font.get_height()+4)
			_s(screen,rect,selected=i==selected)
			_r(screen,font,line,color,(255,255,255),(panel.x+24,y))
			y+=font.get_height()+8
		...
		for i in range(slots):
			...
			line=f'slot {i+1}'
			rect=pygame.Rect(panel.x+16,y-2,panel.width-32,font.get_height()+4)
			_s(screen,rect,selected=i==game.save_selected)
			_r(screen,font,line,color,(255,255,255),(panel.x+24,y))
			y+=font.get_height()+10
		...
		for i,opt in enumerate(options):
			...
			rect=pygame.Rect(panel.x+16,y-2,panel.width-32,font.get_height()+4)
			_s(screen,rect,selected=i==game.leave_selected)
			_r(screen,font,opt,color,(255,255,255),(panel.x+24,y))
			y+=font.get_height()+6
	elif game.ui_mode=='level_skipper':
		available=game.inventory.get('rogue level skipper',0)
		title=tr(game.lang,'msg.skipper_prompt')
		_r(screen,font,title,(230,230,230),(255,255,255),(panel.x+20,y))
		y+=font.get_height()+14
		...
		pygame.draw.rect(screen,(255,255,255),rect,1,border_radius=4)
		pygame.draw.rect(screen,_l(),rect,2,border_radius=4)
		_r(screen,font,line,(255,255,0),(255,255,255),(panel.x+24,y))
		y+=font.get_height()+18
		hint=tr(game.lang,'msg.skipper_hint')
		_r(screen,font,hint,(200,200,200),(255,255,255),(panel.x+20,y))
...
def draw_player_ui(game,screen):
	...
	if player is not None:
		...
		_r(screen,font,hp_text,(255,200,200),(255,255,255),(16,y0))
		_r(screen,font,mp_text,(200,220,255),(255,255,255),(16,y0+surf_hp.get_height()+4))
		hb_font=_q(13,bold=True)
		...
		for i in range(10):
			...
			_r(screen,hb_font,key,(170,190,220),(0,0,0),(sx+2,sy+1),thickness=1)
			name=slots[i]
			if name:
				label=_u(game,name)if game.active_hotbar=='item'else _v(game,name)
				_r(screen,hb_font,_t(hb_font,label,slot_w-6),(230,230,230),(0,0,0),(sx+2,sy+14),thickness=1)
...
def draw_tutorial_panel(game,screen):
	...
	_r(screen,font_title,f'{speaker}:',(170,220,255),(0,0,0),(panel.x+12,panel.y+10),thickness=2)
	_r(screen,font_title,title,(245,245,245),(0,0,0),(panel.x+84,panel.y+10),thickness=2)
	if progress:
		_r(screen,font_body,progress,(255,240,170),(0,0,0),(panel.right-150,panel.y+12),thickness=2)
	lines=_ab(font_body,hint,panel.width-24)
	yy=panel.y+36
	for line in lines[:3]:
		_r(screen,font_body,line,(225,235,245),(0,0,0),(panel.x+12,yy),thickness=1)
		yy+=font_body.get_height()+4
	if countdown is not None:
		ctext=tr(game.lang,'tutorial.dev.countdown',sec=int(countdown))
		_r(screen,font_body,ctext,(255,210,150),(0,0,0),(panel.x+12,panel.bottom-24),thickness=2)
def _aa(screen,panel,game,font):
	...
	centers={}
	rects={}
	anchor_base={'map_1.json':(0.16,0.55),'map_2.json':(0.5,0.55),'map_3.json':(0.84,0.55),'ritc.json':(0.5,0.2),'rogue':(0.84,0.2),'farm_01.json':(0.5,0.83)}
	focus=cur if cur in anchor_base else'map_2.json'
	focus_src=anchor_base.get(focus,(0.5,0.55))
	focus_dst=(0.5,0.55)
	...
	for name,r in rects.items():
		...
		if known:
			...
			if raw=='map_1':
				...
				label=raw
			_r(screen,font,label,(235,245,255),(0,0,0),(r.x-2,r.y-18))
		else:
			_r(screen,font,'未命名',(180,180,180),(0,0,0),(r.x+2,r.y-18))
	if cur in rects:
		r=rects[cur]
	...
	_r(screen,lg_font,'N',(235,235,235),(0,0,0),(cx-7,cy-41))
	_r(screen,lg_font,'S',(235,235,235),(0,0,0),(cx-7,cy+24))
	_r(screen,lg_font,'W',(235,235,235),(0,0,0),(cx-41,cy-10))
	_r(screen,lg_font,'E',(235,235,235),(0,0,0),(cx+25,cy-10))
	leg_font=_q(18,bold=True)
	u=pygame.Rect(lg_x,lg_y+82,24,18)
	pygame.draw.rect(screen,(68,78,88),u)
	pygame.draw.rect(screen,(0,0,0),u,2)
	_r(screen,leg_font,tr(game.lang,'map.unexplored'),(220,220,220),(0,0,0),(u.right+10,u.y-2))
	...
	_r(screen,leg_font,tr(game.lang,'map.current'),(220,220,220),(0,0,0),(c.right+10,c.y-2))
...
def draw_blackjack(game,screen):
	...
	body_font=_q(17)
	small_font=_q(15)
	_r(screen,title_font,tr(game.lang,'blackjack.title'),(245,245,245),(0,0,0),(panel.x+20,panel.y+16),thickness=2)
	bet=int(st.get('bet',0)or 0)
	payout=int(st.get('payout',0)or 0)
	left_money=int(getattr(game,'money',0))
	_r(screen,small_font,f"{tr(game.lang,'blackjack.bet')}: {bet}",(210,230,245),(0,0,0),(panel.right-180,panel.y+22),thickness=2)
	_r(screen,small_font,f"{tr(game.lang,'blackjack.payout')}: {payout}",(210,230,245),(0,0,0),(panel.right-180,panel.y+46),thickness=2)
	_r(screen,small_font,f"{tr(game.lang,'blackjack.left_money')}: {left_money}",(210,230,245),(0,0,0),(panel.right-180,panel.y+70),thickness=2)
	y0=panel.y+78
	...
	_r(screen,body_font,f"{tr(game.lang,'blackjack.dealer')}: {dealer_cards_text}",(235,235,235),(0,0,0),(panel.x+22,y0),thickness=2)
	dealer_value=dealer.get('value',None)
	dealer_value_text='?'if dealer_value is None else str(dealer_value)
	_r(screen,body_font,f"{tr(game.lang,'blackjack.dealer_value')}: {dealer_value_text}",(200,220,240),(0,0,0),(panel.x+22,y0+26),thickness=2)
	_r(screen,body_font,f"{tr(game.lang,'blackjack.you')}: {_fmt_cards(player.get('cards',[]))}",(245,245,220),(0,0,0),(panel.x+22,y0+70),thickness=2)
	_r(screen,body_font,f"{tr(game.lang,'blackjack.your_value')}: {player.get('value',0)}",(245,245,220),(0,0,0),(panel.x+22,y0+96),thickness=2)
	reaction=narrative.get('reaction','')
	final_comment=narrative.get('final_comment','')
	_r(screen,small_font,f"{tr(game.lang,'blackjack.reaction')}: {reaction}",(200,235,220),(0,0,0),(panel.x+22,y0+140),thickness=2)
	if finished:
		_r(screen,small_font,f"{tr(game.lang,'blackjack.result')}: {st.get('result','')}",(255,226,160),(0,0,0),(panel.x+22,y0+170),thickness=2)
		_r(screen,small_font,f"{tr(game.lang,'blackjack.comment')}: {final_comment}",(255,226,160),(0,0,0),(panel.x+22,y0+196),thickness=2)
	options=[tr(game.lang,'blackjack.hit'),tr(game.lang,'blackjack.stand'),tr(game.lang,'blackjack.leave')]if not finished else[tr(game.lang,'blackjack.again'),tr(game.lang,'blackjack.leave')]
	oy=panel.bottom-92
	ox=panel.x+22
	for i,label in enumerate(options):
		...
		_s(screen,rect,selected=i==selected)
		color=(255,247,170)if i==selected else(230,230,230)
		_r(screen,body_font,label,color,(0,0,0),(rect.x+18,rect.y+8),thickness=2)
	_r(screen,small_font,tr(game.lang,'blackjack.hint'),(180,205,225),(0,0,0),(panel.x+22,panel.bottom-34),thickness=2)
def draw_blackjack_bet(game,screen):
	...
	title_font=_q(24,bold=True)
	body_font=_q(18)
	_r(screen,title_font,tr(game.lang,'blackjack.session_title'),(245,245,245),(0,0,0),(panel.x+20,panel.y+16),thickness=2)
	_r(screen,body_font,tr(game.lang,'blackjack.session_prompt'),(225,235,245),(0,0,0),(panel.x+20,panel.y+62),thickness=2)
	_r(screen,body_font,tr(game.lang,'blackjack.session_money',money=int(getattr(game,'money',0))),(200,220,240),(0,0,0),(panel.x+20,panel.y+90),thickness=2)
	value=getattr(game,'blackjack_bet_input','')or''
	box=pygame.Rect(panel.x+20,panel.y+125,panel.width-40,42)
	_s(screen,box,selected=True)
	_r(screen,body_font,value if value else'_',(255,247,170),(0,0,0),(box.x+10,box.y+10),thickness=2)
	err=getattr(game,'blackjack_bet_error','')
	if err=='invalid':
		_r(screen,body_font,tr(game.lang,'blackjack.session_err_invalid'),(255,180,180),(0,0,0),(panel.x+20,panel.y+176),thickness=2)
	elif err=='range':
		_r(screen,body_font,tr(game.lang,'blackjack.session_err_range'),(255,180,180),(0,0,0),(panel.x+20,panel.y+176),thickness=2)
	_r(screen,body_font,tr(game.lang,'blackjack.session_hint'),(180,205,225),(0,0,0),(panel.x+20,panel.bottom-34),thickness=2)
def draw_shop(game,screen):
	...
	if not game.shop_items:
		...
		for i,item in enumerate(game.shop_items):
			...
			line=f"{_u(game,item['name'])} - {item['price']} {tr(game.lang,'label.robux')}"
...
def draw_death_menu(game,screen):
	...
	if getattr(game,'death_no_save_notice',''):
		...
		_r(screen,font,'ok',(230,230,230),(0,0,0),(ok_rect.centerx-10,ok_rect.y+8),thickness=2)
		return
	...
	for i,text in enumerate(options):
		...
		_s(screen,rect,selected=i==selected)
		color=(255,247,170)if i==selected else(230,230,230)
		_r(screen,font,_t(font,text,rect.width-14),color,(0,0,0),(rect.x+8,rect.y+8),thickness=2)
def draw_level_stat_choice(game,screen):
	...
	_r(screen,font,title,(235,245,255),(0,0,0),(box.x+20,box.y+18),thickness=2)
	_r(screen,font2,sub,(190,220,245),(255,255,255),(box.x+20,box.y+50),thickness=2)
	y=box.y+95
	...
	for i,op in enumerate(options):
		...
		_s(screen,rect,selected=i==selected)
		color=(255,247,170)if i==selected else(230,230,230)
		_r(screen,font2,line,color,(0,0,0),(box.x+24,y),thickness=2)
		y+=font2.get_height()+14
	hint=tr(game.lang,'level_stat.hint')
	_r(screen,font2,hint,(180,205,225),(255,255,255),(box.x+20,box.bottom-34),thickness=2)
def draw(game,screen):
	...
	minimap_rect=_p(game,screen,cam_px,cam_py,view_w_px,view_h_px)
	...
	if minimap_rect and tracking_lines:
		...
		yy=box_y+5
		for line in tracking_lines:
			_r(screen,tfont,line,(230,230,230),(0,0,0),(box_x+8,yy),thickness=1)
			yy+=line_h
	...
	if mission_upload:
		...
		_r(screen,upload_font,_t(upload_font,label,bar_w-10),(245,245,245),(0,0,0),(bar_x+6,bar_y-18),thickness=1)
...
def _ac(option,lang=None):
	if isinstance(option,dict):
		lang_l=str(lang or'').lower()
		preferred_keys=('text_zh','label','text','title','name','caption','option')if lang_l=='zh'else('label','text','title','name','caption','option','text_zh')
	...
	return str(option)
def _ad(game,attr_names,default=0):
	for name in attr_names:
		if hasattr(game,name):
			try:
				...
				return default
	return default
def _ae(screen,rect,font,top_more=False,bottom_more=False):
	if top_more:
		_r(screen,font,'↑ More',(210,225,245),(0,0,0),(rect.right-78,rect.y+4),thickness=1)
	if bottom_more:
		_r(screen,font,'↓ More',(210,225,245),(0,0,0),(rect.right-78,rect.bottom-font.get_height()-6),thickness=1)
def _af(game,screen,rect,font):
	...
	_r(screen,font,_t(font,text,box.width-16),(255,240,180),(0,0,0),(box.x+8,box.y+6),thickness=1)
...
def draw_interact_picker(game,screen):
	...
	_r(screen,title_font,'Choose NPC',(240,240,240),(0,0,0),(x+10,y+8),thickness=2)
	selected=_ad(game,('interact_selected','dialog_selected','mission_board_selected'),0)%len(candidates)
	...
	yy=y+30
	for i in range(start,end):
		eid=_ac(candidates[i],getattr(game,'lang',None))
		rect=pygame.Rect(x+8,yy-1,box_w-16,row_h)
		_s(screen,rect,selected=i==selected)
		color=(255,247,170)if i==selected else(230,230,230)
		_r(screen,font,_t(font,eid,rect.width-16),color,(0,0,0),(x+14,yy+2),thickness=2)
		yy+=row_h
	_ae(screen,pygame.Rect(x,y,box_w,box_h),font,top_more=start>0,bottom_more=end<len(candidates))
def draw_dialog(game, screen):
	...
	portrait_frame = portrait_box.inflate(8, 8)
	pygame.draw.rect(screen, (10, 16, 26), portrait_frame, border_radius=10)
	pygame.draw.rect(screen, (180, 220, 255), portrait_frame, 1, border_radius=10)

	speaker_font = _q(16, bold=True)
	body_font = _q(15)
	opt_font = _q(14)

	title = str(
		getattr(game, "dialog_speaker_name", "")
		or getattr(game, "dialog_npc_name", "")
		or getattr(game, "dialog_title", "")
		or getattr(game, "active_npc", "")
		or "NPC"
	)
	...
	portrait = _k(game, npc_id, ent_def, (portrait_box.width, portrait_box.height))

	if portrait is not None:
		px = portrait_box.centerx - portrait.get_width() // 2
		py = portrait_box.centery - portrait.get_height() // 2
		screen.blit(portrait, (px, py))
	else:
		label = _t(speaker_font, title or npc_id or "NPC", portrait_box.width - 12)
		_r(
			screen,
			speaker_font,
			label,
			...
			(
				portrait_box.x + 8,
				portrait_box.centery - speaker_font.get_height() // 2,
			),
			thickness=2,
		)

	name_lines = _ab(speaker_font, title or npc_id or "NPC", max(80, name_box.width - 8))
	name_lines = name_lines[:2] or ["NPC"]

	name_h = len(name_lines) * (speaker_font.get_height() + 2)
	name_y = name_box.centery - name_h // 2

	for line in name_lines:
		text = _t(speaker_font, line, name_box.width - 8)
		text_w = speaker_font.size(text)[0]
		text_x = name_box.centerx - text_w // 2
	...
	node = _resolve_dialog_node(
		getattr(game, "dialog_data", None),
		getattr(game, "dialog_node", None),
	)
	...
	elif isinstance(option_source, (list, tuple)):
		options = list(option_source)
	else:
	...
	if isinstance(text_source, str):
		for part in text_source.splitlines() or [text_source]:
			dialog_lines.extend(_ab(body_font, part, body_width))
	else:
		for item in list(text_source or []):
			dialog_lines.extend(_ab(body_font, str(item), body_width))
	...
	option_rows = min(len(options), max_visible_options) if options else 0
	option_area_h = option_rows * opt_row_h + 18 if option_rows else 0

	text_top = right.y + 8
	text_bottom = right.bottom - option_area_h - 10
	line_h = body_font.get_height() + 4

	max_text_lines = max(1, (text_bottom - text_top) // line_h)

	text_scroll = int(getattr(game, "dialog_scroll", 0) or 0)
	text_scroll = max(0, min(text_scroll, max(0, len(dialog_lines) - max_text_lines)))
	setattr(game, "dialog_scroll", text_scroll)

	text_start = text_scroll
	text_end = min(len(dialog_lines), text_start + max_text_lines)

	yy = text_top

	for line in dialog_lines[text_start:text_end]:
		_r(
			screen,
			body_font,
			line,
	...
	_ae(
		...
		right,
		body_font,
		top_more=text_start > 0,
		bottom_more=text_end < len(dialog_lines),
	)
	...
	setattr(game, "dialog_selected", selected)

	page = selected // max_visible_options
	start_opt = page * max_visible_options
	end_opt = min(len(options), start_opt + max_visible_options)

	visible_rows = end_opt - start_opt
	opt_top = right.bottom - visible_rows * opt_row_h - 10

	for i in range(start_opt, end_opt):
		opt = options[i]
		label = _ac(opt, getattr(game, "lang", None))

		rect = pygame.Rect(
			right.x + 4,
			opt_top + (i - start_opt) * opt_row_h,
			right.width - 8,
			opt_row_h - 2,
		)
		...
		_r(
			screen,
			opt_font,
			_t(opt_font, label, rect.width - 14),
			color,
			(0, 0, 0),
			(rect.x + 8, rect.y + 4),
			thickness=2,
	...
	option_view = pygame.Rect(
		right.x,
		opt_top,
		right.width,
		right.bottom - opt_top,
	)
	...
	_ae(
		...
		option_view,
		opt_font,
		top_more=start_opt > 0,
		bottom_more=end_opt < len(options),
	)
def draw_mission_board(game,screen):
	...
	pygame.draw.rect(screen,(180,220,255),panel,2,border_radius=12)
	title_font=_q(20,bold=True)
	body_font=_q(15)
	small_font=_q(13)
	_r(screen,title_font,tr(game.lang,'mission.board.title'),(245,245,250),(0,0,0),(panel.x+18,panel.y+14),thickness=2)
	_af(game,screen,panel,body_font)
	...
	if not entries:
		...
		_r(screen,body_font,empty,(230,230,230),(0,0,0),(panel.x+18,panel.y+58),thickness=2)
		return
	...
	for i in range(start,end):
		...
		color=(255,247,170)if i==selected else(230,230,230)
		_r(screen,list_font,_t(list_font,label,rect.width-14),color,(0,0,0),(rect.x+8,rect.y+5),thickness=2)
		yy+=row_h
	_ae(screen,left,list_font,top_more=start>0,bottom_more=end<len(entries))
	...
	if mission_name:
		_r(screen,title_font,_t(title_font,mission_name,right.width-20),(250,250,250),(0,0,0),(right.x+10,dy),thickness=2)
		dy+=title_font.get_height()+8
	if giver:
		_r(screen,detail_small,giver,(205,225,245),(0,0,0),(right.x+10,dy),thickness=2)
	...
	if status not in('briefing','ready_to_return','available','ready','active','completed'):
		...
		_r(screen,detail_small,_t(detail_small,status_reason,right.width-20),(245,205,170),(0,0,0),(right.x+10,dy),thickness=2)
	...
	if not detail_blocks:
		...
		if desc:
			detail_blocks.append(('description',_ab(detail_font,str(desc),right.width-24)))
	block_titles={'description_lines':tr(game.lang,'mission.board.briefing'),'accept_lines':tr(game.lang,'mission.board.available'),'objective_lines':tr(game.lang,'mission.board.list'),'reward_lines':tr(game.lang,'mission.board.rewards'),'return_lines':tr(game.lang,'mission.board.completion'),'description':tr(game.lang,'mission.board.briefing')}
	...
	for key,value in detail_blocks:
		...
		for line in value:
			if isinstance(line,str):
				detail_lines.extend(_ab(detail_font,line,right.width-24))
			else:
				detail_lines.extend(_ab(detail_font,str(line),right.width-24))
		detail_lines.append('')
	...
	for line in detail_lines[dstart:dend]:
		...
		if line.endswith(':'):
			color=(200,220,245)
		_r(screen,detail_font,line,color,(0,0,0),(right.x+10,yy),thickness=2)
		yy+=line_h
	_ae(screen,right,detail_font,top_more=dstart>0,bottom_more=dend<len(detail_lines))
	hint=tr(game.lang,'mission.board.hint')
	_r(screen,small_font,hint,(180,205,225),(0,0,0),(panel.x+18,panel.bottom-28),thickness=2)
def _dialog_layout(screen):
	w,h=(screen.get_width(),screen.get_height())
	ph=max(180,h//4)
	panel=pygame.Rect(0,h-ph,w,ph)
	left=pygame.Rect(panel.x+8,panel.y+8,max(130,w//4),panel.height-16)
	portrait=pygame.Rect(left.x+8,left.y+8,left.width-16,max(80,left.height-48))
	name=pygame.Rect(left.x+8,portrait.bottom+6,left.width-16,max(24,left.bottom-portrait.bottom-14))
	right=pygame.Rect(left.right+10,panel.y+10,panel.right-left.right-20,panel.height-20)
	return{'panel':panel,'left':left,'portrait':portrait,'name':name,'right':right}
def _resolve_dialog_node(data,node_id):
	if isinstance(data,dict):
		...
		if isinstance(n,list):
			for x in n:
				if isinstance(x,dict)and x.get('id')==node_id:
					return x
	return None
